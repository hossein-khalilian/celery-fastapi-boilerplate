...
export default function Home(){
  ...
  async function submit(e){
    ...
    try {
      ...
      setStatus('FAILURE')
      setResult({error: `Failed to submit task: ${error.message}. Make sure the backend is running at ${API_BASE}`})
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold text-slate-900">Text Processing Demo</h1>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Enter your text below and it will be processed asynchronously using Celery workers
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        {taskId && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Task ID: <code className="text-xs bg-slate-100 px-2 py-1 rounded">{taskId}</code>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-primary">{progress !== null ? progress : 0}%</span>
                </div>
                <Progress value={progress !== null ? progress : 0} className="h-3" />
                {status === 'PENDING' && progress === 0 && (
                  <p className="text-sm text-muted-foreground italic">Waiting for task to start...</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              {result.elapsed_time !== undefined && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-semibold text-blue-900">Total Processing Time</p>
